...
class Color:
    ...
    def color(self, value):
        ...
        if CREGEX.PALETTE_COLOR_RE.search(value):
            if not self.palette:
                raise ValueError(
    ...
    def decode_sgr(source_color_code, is_reset=False):
        ...
        if not CREGEX.SGR_COLOR_RE.search(source_color_code):
    ...
    def strip_colors(data):
        ...
        '''
        inserts = []
        match = CREGEX.SGR_RE.search(data)
...
class Palette:
    ...
    def resolve(self, color):
        ...
        for match in reversed(list(CREGEX.PALETTE_COLOR_RE.finditer(color))):
            start, end = match.span()
